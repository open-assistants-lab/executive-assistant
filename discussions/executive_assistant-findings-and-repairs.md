# Executive Assistant Findings and Repairs
...
- Local run instructions requiring `uv run executive_assistant` and `docker compose up -d postgres`.
...
- In `src/executive_assistant/channels/base.py` and `src/executive_assistant/channels/telegram.py`, the state was built as:
  - `{"messages": [...]}`
...
2) Run Executive Assistant locally:
```bash
uv run executive_assistant
```
