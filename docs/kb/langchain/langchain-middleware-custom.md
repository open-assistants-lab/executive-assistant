# Custom Middleware - LangChain Docs
...
Run sequentially at specific execution points. Use for logging, validation, and state changes.
...
Intercept execution and control when the handler is called. Use for retries, caching, and transformation. You decide if the handler is called zero times (short-circuit), once (normal flow), or multiple times (retry logic).
