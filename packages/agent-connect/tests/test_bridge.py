...
def temp_dir(monkeypatch):
    d = tempfile.mkdtemp()
    monkeypatch.setenv("AGENT_CONNECT_SPEC_DIR", str(Path(d) / "connectors"))
    from cryptography.fernet import Fernet
    monkeypatch.setenv("AGENT_CONNECT_VAULT_KEY", Fernet.generate_key().decode())
    # Use temp dir for vaults too
    from agent_connect.bridge import _default_vault_path as _orig

    def _test_vault_path(user_id: str) -> str:
        return str(Path(d) / "vaults" / user_id)

    import agent_connect.bridge as bridge_module
...
class TestBridgeLifecycle:
    ...
    async def test_discover_no_connectors(self, temp_dir):
        ...
        bridge = AgentConnectBridge("alice")
    ...
    async def test_discover_with_connected(self, temp_dir):
        ...
        with patch("agent_connect.backends.cli.CLIAdapter.list_commands",
...
class TestConnectedServices:
    def test_lists_connected(self, temp_dir):
        ...
        bridge = AgentConnectBridge("user_conn_svc")
        bridge.vault.store_token("github", "api_key", {"api_key": "ghp_123"})
    ...
    def test_empty(self, temp_dir):
        _write_connectors(temp_dir, [_make_cli_spec("github")])
        bridge = AgentConnectBridge("newuser")
