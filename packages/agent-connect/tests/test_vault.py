...
from agent_connect.vault import CredentialVault, _get_or_create_key


@pytest.fixture
def vault(request, monkeypatch, tmp_path):
    monkeypatch.setenv("AGENT_CONNECT_VAULT_KEY", "")
...
def vault_with_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("AGENT_CONNECT_VAULT_KEY", key)
    v = CredentialVault(str(tmp_path))
    yield v
    v.close()
    shutil.rmtree(str(tmp_path), ignore_errors=True)
...
class TestKeyGeneration:
    ...
    def test_generates_ephemeral_when_missing(self, monkeypatch):
        monkeypatch.delenv("AGENT_CONNECT_VAULT_KEY", raising=False)
        with pytest.warns(UserWarning, match="AGENT_CONNECT_VAULT_KEY not set"):
