#!/usr/bin/env python
"""Test simplified VDB tools interface.

This validates that the new simplified parameters work correctly:
- create_vdb_collection with content parameter (single document)
- create_vdb_collection with documents parameter (JSON array)
- create_vdb_collection empty, then add_vdb_documents with content
- search_vdb for semantic search
"""
...
def test_simplified_vdb_interface():
    """Test the simplified VDB tools with content parameter."""
    ...
    print(f"   Result: {result}")
    assert "Created VDB collection" in result and "2 document" in result, result
    print("   ✅ JSON array method still works (backward compatible)")

    # Cleanup
    print("\n[Cleanup] Removing test collections...")
    drop_vdb_collection.invoke({"collection_name": collection_name})
    drop_vdb_collection.invoke({"collection_name": f"{collection_name}_json"})
    print("   ✅ Test collections removed")

    print("\n" + "=" * 60)
    print("✅ All VDB simplified interface tests passed!")
    print("=" * 60)
...
def test_empty_collection_workflow():
    ...
    print("\n" + "=" * 60)
    print("VDB Empty Collection Workflow Test")
    print("=" * 60)
