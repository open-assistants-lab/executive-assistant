...
class TestHybridSearchPerformance:
    ...
    def test_all_hybrid_search_scenarios(self, storage):
        ...
        for name, query in sql_tests:
            start = time.time()
            results = storage.query_sql(app_name, query)
        ...
        for term, column in fts_tests:
            start = time.time()
            try:
                results = storage.search_fts(app_name, "books", column, term, limit=20)
                elapsed = (time.time() - start) * 1000
        ...
        books = storage.query_sql(app_name, "SELECT id FROM books LIMIT 20")
        ...
        for limit in [10, 25, 50, 100]:
            start = time.time()
            results = storage.query_sql(app_name, f"SELECT * FROM books ORDER BY id LIMIT {limit}")
            elapsed = (time.time() - start) * 1000
            print(f"   LIMIT {limit}: {len(results)} rows in {elapsed:.2f}ms")

        # Verify FTS5 works by checking results
        fts_results = storage.search_fts(app_name, "books", "full_text", "journey", limit=10)
